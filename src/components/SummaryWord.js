const SummaryWord = ({ word_json, SummaryOnMouseDownID, StateMachineState, SummaryMouseClickHandlerWrapper, hoverHandlerWrapper, SummaryOnMouseDownHandler, SummaryOnMouseUpHandler, setSummaryOnMouseDownActivated, summaryOnMouseDownActivated, setHoverActivatedId, ctrlButtonDown, setHoverActivatedDocOrSummary,  CurrSentInd, currAlignmentToShow}) => {
              ...
              {(word_json.word !== "\n") && (
                <div
                  ...
                  className={`summaryWord noselect normal-sized-word  text-muted
                              ${((word_json.span_highlighted && !word_json.span_alignment_hover) || (word_json.alignment_id.includes(currAlignmentToShow))) ? 'span-highlighted-word': ''} // if the word was span_highlighted before and now go over it again, then should unspan it
                              ${(word_json.span_alignment_hover && !ctrlButtonDown) ?  'span-aligned-hover-word': ''} 
                              ${(word_json.old_alignments && !word_json.span_highlighted && !word_json.alignment_id.includes(currAlignmentToShow)) ? 'old-aligned-word': ''}
